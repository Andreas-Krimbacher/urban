...
Timeline.OriginalEventPainter.prototype.paint = function() {
   ...
   impreciseIconMargin: eventTheme.instant.impreciseIconMargin
    ...
    var iterator = eventSource.getEventIterator(minDate, maxDate); //changed: removed inverse
...
Timeline.OriginalEventPainter.prototype.paintPreciseDurationEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    var labelTop = Math.round(
        metrics.trackOffset + track * metrics.trackIncrement - theme.event.label.bottomMargin); //changed: display title inside of the tape
    ...
    this._tracks[track] = {end : endPixel,rank : evt.getRank()};
};
...
Timeline.OriginalEventPainter.prototype._findFreeTrack = function(event, leftEdge) {
    ...
    for (var i = 0; i < this._tracks.length; i++) {
        var t = this._tracks[i].end;
        if (t < leftEdge && event.getRank() == this._tracks[i].rank) {
            break;
        }
    }
//    var i = this._tracks.length;
//    if(this._tracks[this._tracks.length-1].end < leftEdge) i--;
    return i;
};
