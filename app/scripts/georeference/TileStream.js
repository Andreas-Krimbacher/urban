 ...
 * Example:
 * (code)
 *     var layer = new OpenLayers.Layer.TileStream(
...
OpenLayers.Layer.TileStream = OpenLayers.Class(OpenLayers.Layer.Grid, {
     ...
     *    'href="http://TileStream.osgeo.org/1.0.0/vmap0"', the <layername> property
     ...
     *
     * Returns:
     * {<OpenLayers.Layer.TileStream>} An exact clone of this <OpenLayers.Layer.TileStream>
     */
    clone: function (obj) {

        if (obj == null) {
            obj = new OpenLayers.Layer.TileStream(this.name,
                this.url,
                this.getOptions());
        }
    ...
    getURL: function (bounds) {
        ...
        var x = Math.round((bounds.left - this.map.getMaxExtent().left) / (res * this.tileSize.w));

        var y = Math.round((this.map.getMaxExtent().top - bounds.top) / (res * this.tileSize.h));
    ...
    CLASS_NAME: "OpenLayers.Layer.TileStream"
});
